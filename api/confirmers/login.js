...
const Confirmer = require('../../model/Confirmer');
...
router.post('/', async (request, response) => {
    ...
    if (username && password) {
        ...
        const findResult = await Confirmer.findOne({ username });
        if (findResult) {
            const passwordInDb = findResult.password;

            if (hash.verify(password, passwordInDb)) {
                try {
                    const token = jwt.sign({ username, role:"confirmers", type: 'token', id: findResult._id }, vToken.JWTprivateKey, { expiresIn: '30d', algorithm: 'RS256' });
                    const refreshToken = jwt.sign({ username, role: "confirmers", type: 'refresh', id: findResult._id }, vToken.JWTprivateKey, { expiresIn: '60d', algorithm: 'RS256' });
                    const packet = {
                        "token": "bearer " + token,
